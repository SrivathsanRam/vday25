...
const WordleInput: React.FC<Props> = ({
  ...
  const [currentGuess, setCurrentGuess] = useState<string[]>([...Array(6)]);
  ...
  const handleKeyUp = (e: React.KeyboardEvent, i: number): void => {
    ...
    currentGuess.filter((letter: string) => letter && letter).length === 6;
    if (e.key === "Backspace") {
      let inputIndex: number = i - 1 >= 0 ? i - 1 : i;
      autoTab(inputIndex, index);
    } else if (i === 5 && e.key === "Enter" && isCurrentGuessFull) {
      handleSubmit();
      autoTab(0, index + 1);
    } else {
      autoTab(i + 1, index);
  ...
  return (
    <div>
      {currentGuess.map(
        (_letter: string, i: number): JSX.Element => (
          <input
            key={i}
            id={`${i}${index}`}
            type="text"
